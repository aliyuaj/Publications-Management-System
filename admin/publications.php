...
<title>Admin | All Publications List</title>
	...
	<div class="starter-template">
		...
		<div class="col-md-3 hidden-print" style="padding-left:0px;">
			<form role="form" >
				...
				<?php					/****ALL PUBLICATIONS****/
					echo '<div id="all">';
                    $per_page=20;
                    $pages_query=mysqli_num_rows(mysqli_query($con,"SELECT * FROM materials"));
                    if($pages_query>0){
					...
					$query5=mysqli_query($con,"SELECT * from materials JOIN categories ON category=catID LIMIT $start,$per_page");
					$count=mysqli_num_rows($query5);
					echo "<div style='font-family:Palatino Linotype;font-size:30px;color:green;' align='left'><b>All Publications: </b>$pages_query</div>
					<div  class='table-responsive' >
					<table  class='table table-striped table-condensed table-bordered' style='width:100%' >
					<thead>
					<tr>
					<td><b>S/No.</b></td><td><b>Title</b></td><td><b>Authors</b></td><td><b>Sponsors</b></td><td class='hidden-print'><b>Copies</b></td><td><b>Category</b></td><td class='hidden-print'><b>Location</b></td><td class='hidden-print'></td>
					</tr> </thead>";
					$i=($per_page*($page-1))+1;
                        while($result=mysqli_fetch_assoc($query5)){
                        echo "<tr><td >".$i."</td><td align='justify'>".$result['title']."</td><td align='justify'>".$result['author']."</td><td align='justify'>".$result['sponsor']."</td><td class='hidden-print'>".$result['copies']."</td><td align='justify'>".$result['catName']."</td><td class='hidden-print' >".$result['location'].'</td><td class="hidden-print">
					<div class="btn-group ">
			...
			if($pages>1 && $page<=$pages){
                ...
                echo "<h2 style='color:red;font-family:\"Times New Roman\", Times, serif;'>Publication is currently empty</h2>";			}
			echo'</div>';
