...
if (isset($_POST['save'])){
    ...
    echo $sub_subject=$_POST['subsubj'];
    echo $sub_sub_subject=$_POST['subsubsubj'];
	$cat=$_POST['category'];
	$year=$_POST['year'];
	$synopsis=ucfirst(trim($_POST['synopsis']));
	$month=$_POST['month'];
	$date=$_POST['date'];
	$update=mysqli_query($con,"UPDATE materials SET copies='$copies',title='$title',author='$author',subject='$subject',sub_subjects='$sub_subject',sub_sub_subjects='$sub_sub_subject',sponsor='$sponsor',language='$lang',pubNum='$pnum', synopsis='$synopsis',pubYear='$year', category='$cat', month='$month',
	...
	$aid=$_SESSION['id'];
		...
		$_SESSION['done']="Unable to update publication ".mysqli_error($con);
		header('location:edit.php?id='.$id);
	}
}
?> 