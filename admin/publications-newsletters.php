...
<head>
    ...
    <title>Publications: Admin Publications - Agric. Newsletters</title>
...
<div class="container" id="cc">
	<div class="starter-template">
	   ...
	   <?php					/****ALL BULLETINS****/
		echo '<div id="all">';
	$per_page=20;
	$pages_query=mysqli_num_rows(mysqli_query($con,"SELECT * FROM materials JOIN categories ON category=catID WHERE catName='Agric. Newsletter'"));
	if($pages_query>0) {
        ...
        $query5 = mysqli_query($con, "SELECT * FROM materials JOIN categories ON category=catID WHERE catName='Agric. Newsletter' LIMIT $start,$per_page");
        $count = mysqli_num_rows($query5);
        echo "<div style='font-family:Palatino Linotype;font-size:30px;color:green;' align='left'><b>Agric. Newsletters: </b>$pages_query</div>
	...
	<td><b>S/No.</b></td><td><b>Title</b></td><td><b>Authors</b></td><td><b>Sponsors</b></td><td class='hidden-print'><b>Copies</b></td><td class='hidden-print'><b>Location</b></td><td class='hidden-print'></td>
        ...
        while ($result = mysqli_fetch_assoc($query5)) {
            echo "<tr><td >" . $i . "</td><td align='justify'>" . $result['title'] . ' (' . $result['pubYear'] . ')' . "</td><td align='justify'>" . $result['author'] . "</td><td align='justify'>" . $result['sponsor'] . "</td><td class='hidden-print'>" . $result['copies'] . "</td><td class='hidden-print'>" . $result['location'] . '</td><td class="hidden-print">
				<div class="btn-group ">
	...
	<ul class="dropdown-menu" role="menu">
		...
		<li><ahref="#delete" data-toggle="modal" data-del="' . $result["matID"] . '"><span class="glyphicon glyphicon-trash"></span> Remove</a></li>
		';
            echo '</ul>
	</div></td>
	</tr>';
