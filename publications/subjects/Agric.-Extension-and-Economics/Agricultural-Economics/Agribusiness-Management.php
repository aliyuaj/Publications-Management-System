...
<div class="col-md-6">
    <div>
        ...
        ?>
        <div style='font-family:"Trebuchet MS", Helvetica, sans-serif;font-size:30px;color:green;'><b> Agribusiness Management </b></div>
        <?php
        $per_page=5;
        $pages_query=mysqli_num_rows(mysqli_query($con,"SELECT * FROM materials left join categories ON materials.category=categories.catID join sub_sub_subjects ON materials.sub_sub_subjects=sub_sub_subjects.sssID WHERE sub_sub_subjects.sssName='Irrigation Agronomy' "));
        ...
        $query5=mysqli_query($con,"SELECT * from materials left join categories ON materials.category=categories.catID join sub_sub_subjects ON  materials.sub_sub_subjects=sub_sub_subjects.sssID WHERE sub_sub_subjects.sssName='Irrigation Agronomy'   ORDER BY today DESC LIMIT $start,$per_page");
