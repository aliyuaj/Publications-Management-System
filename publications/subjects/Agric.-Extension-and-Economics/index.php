<?php
$title = 'Publication: Agric Economics';
$location = '../../../';
$publocation = '../../';
...
<div class="col-md-6">
    <div>
        ...
        ?>
        <div style='font-family:"Trebuchet MS", Helvetica, sans-serif;font-size:30px;color:green;'><b> Agric Economics </b></div>
        <?php
        $per_page=5;
        $pages_query=mysqli_num_rows(mysqli_query($con,"SELECT * FROM materials left join categories ON materials.category=categories.catID left join subjects ON materials.subject=subjects.subjectID WHERE subjectName='agric-economics' "));
        ...
        $query5=mysqli_query($con,"SELECT * from materials left join categories ON materials.category=categories.catID left join subjects ON materials.subject=subjects.subjectID WHERE subjectName='agric-economics' LIMIT $start,$per_page");
 ...
 <input type='text' value=".$id." name='mid' style='display:none'/>";
     ...
     <input type="text" value="'.$file_name.'" name="file" style="display:none"/>
        ...
        if you already have an account or <a href='".$location."register.php'>register</a> to make request</div>";
            ...
            $f = $_POST['file'];
            if(isset($user)) {
                header('location:'.$publocation.'download.php?file_name='. $f);
            }else{
                $_SESSION['request'] = "
