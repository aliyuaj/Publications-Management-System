...
<div class="col-md-6">
    <div>
        ...
        ?>
        <div style='font-family:"Trebuchet MS", Helvetica, sans-serif;font-size:30px;color:green;'><b> Environmental Science </b></div>
        <?php
        $per_page=5;
        $pages_query=mysqli_num_rows(mysqli_query($con,"SELECT * FROM materials left join categories ON materials.category=categories.catID join sub_subjects ON materials.sub_subjects=sub_subjects.ssID WHERE ssName='Environmental Science' "));
        ...
        $query5=mysqli_query($con,"SELECT * from materials left join categories ON materials.category=categories.catID join sub_subjects ON materials.sub_subjects=sub_subjects.ssID WHERE ssName='Environmental Science'  ORDER BY today DESC LIMIT $start,$per_page");
